...
function plotSquareSet(S, color) {
    for (let s of S)
        plotCircle(row(s), col(s), color);
}
function paintBoard() {
    for (let s of all_squares) {
        if ((row(s) + col(s)) % 2 == 0)
            board_color[row(s)][col(s)] = "#D2B48C";
        else
            board_color[row(s)][col(s)] = "#E5D3B3";
    }
}
function plotCircle(row, col, color) {
    pen.strokeStyle = color;
    pen.lineWidth = circle_thickness;
    pen.beginPath();
    pen.arc((col + 0.5) * pixel_width, (row + 0.5) * pixel_height, pixel_width / 2 - 1, 0, 2 * Math.PI, true);
    pen.stroke();
}
