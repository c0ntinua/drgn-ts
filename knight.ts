function updateLegalKnight() {
    ...
    for (let s of knight_set) {
        ...
        if (!invulnerable_pieces.has(pieceAt(s)) && !myPiece(s)) legal_set.add(s);
    }
}
...
function updateKnightSet() {
    ...
    for (let m = 0; m < col_mod.length; m++) {
        let candidate = asSquare(
            row(target_square) +row_mod[m],
            col(target_square) + col_mod[m]
        )
        if (validSquare(candidate)) knight_set.add(candidate);        
    }
}
